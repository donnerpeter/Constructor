...
interactMites leftMite rightMite = case (leftMite, rightMite) of
  ...
  (Conjunction v _, Argument Nom child) -> left [semV v "member2" child, mite $ SeqRight v]
  (Argument Nom child, SeqRight v) -> right [semV v "member1" child, mite $ SeqFull v, mite $ Argument Nom v]
  _ -> []
