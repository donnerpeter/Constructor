...
interactMites leftMite rightMite = case (leftMite, rightMite) of
  ...
  (Word _ ",", condComp@(ConditionComp cp s True)) -> left [mite $ CommaSeparated condComp]
  (Verb head, CommaSeparated (ConditionComp cp cond _)) -> left [semV head (cond++"Condition") cp]
  _ -> []
